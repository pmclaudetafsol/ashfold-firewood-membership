...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ...
    className={cn(
      ...
      'sm:bottom-auto sm:top-0 sm:flex-col md:max-w-[26rem]',
      className,
    )}
    {...props}
  />
...
const toastVariants = cva(
  cn(
    ...
    'rounded-lg border p-4 pr-10 shadow-elevated transition-all',
  ...
  ),
  {
    variants: {
      variant: {
        ...
        information: 'border-information/25 bg-information-muted text-foreground',
      },
    },
...
/** Icons carry the status alongside colour, never instead of it. */
const toastIcons = {
  default: Info,
...
>(({ className, variant = 'default', children, ...props }, ref) => {
  ...
  return (
    <ToastPrimitives.Root ref={ref} className={cn(toastVariants({ variant }), className)} {...props}>
      <Icon className={cn('mt-0.5 size-5 shrink-0', toastIconColours[variant ?? 'default'])} aria-hidden="true" />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      'inline-flex h-8 shrink-0 items-center justify-center rounded-md border border-border-strong',
      'bg-transparent px-3 text-sm font-medium transition-colors hover:bg-card',
